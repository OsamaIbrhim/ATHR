...
export function FieldError({ children }: { children?: ReactNode }) {
  return children
    ? <div className="field-error" role="alert">{children}</div>
    : null
}
...
export function ConfirmDialog({
  ...
  return (
    <Modal open={open} title={title} onClose={onClose} width="460px">
...
> {
  ...
  static getDerivedStateFromError(error: Error) {
    return { error: error.message || 'Unexpected UI error' }
  }

  componentDidCatch(error: Error) {
    console.error('[POS UI]', error)
  }
      ...
      return (
        <main className="auth-shell">
          <section className="auth-card">
            <div className="brand-mark error">!</div>
            <h1>تعذر تشغيل الواجهة</h1>
