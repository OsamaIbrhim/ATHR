...
function fixture() {
  const directory = mkdtempSync(path.join(tmpdir(), 'bold-pos-release-'))
  const packageFile = path.join(directory, 'package.json')
  const lockFile = path.join(directory, 'package-lock.json')
  const installer = path.join(directory, 'Bold-POS-Setup-1.3.3.exe')
...
test('manifest contains a direct immutable release URL and installer checksum', async () => {
  ...
  assert.equal(manifest.url, 'https://github.com/OsamaIbrhim/bold_system/releases/download/pos-v1.3.3/Bold-POS-Setup-1.3.3.exe')
