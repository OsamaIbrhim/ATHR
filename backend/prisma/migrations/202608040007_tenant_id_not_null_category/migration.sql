-- WP-007 Phase B (MT-MIG-006): tenant_id NOT NULL for "Category".
...
DECLARE
  violating INT;
BEGIN
  SELECT count(*) INTO violating FROM "Category" WHERE "tenant_id" IS NULL;
  IF violating <> 0 THEN
    RAISE EXCEPTION 'MT-MIG-006 precondition failed: % row(s) in "Category" have tenant_id IS NULL.', violating;
  END IF;
END $$;
