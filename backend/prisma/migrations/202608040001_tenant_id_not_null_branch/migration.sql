-- WP-007 Phase B (MT-MIG-006): tenant_id NOT NULL for "Branch".
...
DECLARE
  violating INT;
BEGIN
  SELECT count(*) INTO violating FROM "Branch" WHERE "tenant_id" IS NULL;
  IF violating <> 0 THEN
    RAISE EXCEPTION 'MT-MIG-006 precondition failed: % row(s) in "Branch" have tenant_id IS NULL.', violating;
  END IF;
END $$;
